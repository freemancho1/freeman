...
class PrintLogs(threading.Thread):
    ...
    def print_log(log, pre_color):
        ...
        if _level != '' and LOG_LEVEL[_level] < LOG_LEVEL[SYS_CONF['log_level_']]: return

        log_color = LOG_COLOR['   DEBUG'] if pre_color is None else pre_color
        for key in LOG_COLOR.keys():
            if key in log: log_color = LOG_COLOR[key]
        ...
        print(log_color + log + LOG_COLOR_END)
        return log_color
...
def init_parameter():
    ...
    if not os.path.exists(log_file):
        raise Exception(f"로그파일이 존재하지 않습니다.\n{log_file}")
        
    log_level = f"{options.level.upper(): >8}"
    if log_level not in LOG_COLOR.keys():
    ...
    try: 
        ...
        raise Exception(f"초기 출력값은 0 이상의 정수만 입력할 수 있습니다.")
